...
class RecoveryApp(tk.Tk, DrivesMixin, ScanMixin, PreviewMixin, ExportMixin, TreeMixin, LayoutMixin):
    def __init__(self):
        ...
        self.tree_item_map = {}
        self.tree_categories_map = {}
        self.tree_size_groups_map = {}
    ...
    def update_ui_counters(self):
        total = len(self.virtual_files)
        total_size = sum(f["size"] for f in self.virtual_files)
        self.all_files_btn.config(text=f"📁 Tüm Dosyalar ({total} - {self.format_size(total_size)})")
        
        counts = {c: 0 for c in CATEGORIES.keys()}
        sizes = {c: 0 for c in CATEGORIES.keys()}
        ...
        for cat_name, val in counts.items():
            icon = CATEGORIES[cat_name]["icon"]
            cat_size = sizes[cat_name]
